
class Movies {
    static list = [
        {
            title: 'Schindlers List',
            image: require('./SchndilersList.jpg'), // Replace with actual image path
            url: 'https://mega.nz/file/VT8TCKiK#KyB51V9-SlkNZ9zH_KA4htTQQ8JhxxZGaAInfUqQW_w', 
          },
          {
            title: 'The Bucket List',
            image: require('./TheBucketList.jpg'), // Replace with actual image path
            url: 'https://mega.nz/file/ZaF3UAIZ#Nl4Y4aGzFHRxBUFa7nFogDMD13q5iyRKWhqXacmFs7Y', 
          },
          {
            title: 'American Psycho',
            image: require('./AmericanPsycho.jpg'), // Replace with actual image path
            url: 'https://mega.nz/file/Qa9ADCRa#WkxD7OFI9GBnDcyUkBf4sqxudha49ldUTdkk5i2ouro', 
          },
          {
            title: 'The Grand Budapest',
            image: require('./GrandBudapest.jpg'), // Replace with actual image path
            url: 'https://mega.nz/file/sDF1xTiD#MLU6nrGHzYCnqPCofcpeSoKc-LSzhsaji7Pmophu1Uk', 
          },
          {
            title: 'Gone Girl',
            image: require('./GoneGirl.jpg'), // Replace with actual image path
            url: 'https://mega.nz/file/ta0nUITD#Ji45gbNnECDcq7ICKY0Rehp5wkk96AoWWt-kBq6Sx6I', 
          },
          {
            title: 'The Taxi Driver',
            image: require('./TaxiDriver.webp'), // Replace with actual image path
            url: 'https://mega.nz/file/5OEBDAIR#tvBHXHfKHDCkA7q6ECsL6HHwo8mUi_Bt_yKUsLIFWnA', 
          },
          {
            title: 'Prisoners',
            image: require('./Prisoners.jpg'), // Replace with actual image path
            url: 'https://mega.nz/file/4XtSEBTK#Y3GG2bVqpuEYe9yL5ZhcDjYqVFIzv6D_tUlt1GrXTYo', 
          },
          {
            title: 'The Shining',
            image: require('./TheShining.jpg'), // Replace with actual image path
            url: 'https://mega.nz/file/Ae1zjSxR#ox7gSDGaKmQXEDnlQWnOwc9hhGSTompdlwkGJ3DxLV8', 
          },
          {
            title: 'One Flew Over the Cuckoos Nest',
            image: require('./cukkosNest.jpg'), // Replace with actual image path
            url: 'https://mega.nz/file/NHkQWQwB#uIz2irRFkxvaXjW8TtvgaXiSaVk89jGH0Y0CedrT-BM', 
          },
          {
            title: 'Sound of Metal',
            image: require('./SoundOfMetal.jpg'), // Replace with actual image path
            url: 'https://mega.nz/file/Va8zWQAD#hdDsGNtroRrHNaacA-NpCz879hS3oNQtNnr62kFUcHo', 
          },
          {
            title: '1917',
            image: require('./1917.jpg'), // Replace with actual image path
            url: 'https://mega.nz/file/MP1Q3AKC#aNP3SQWjXaYj0sOgaE3vqjwcAKrNqgTzU3rMpxM8Avg', 
          },
          {
            title: 'Fight Club',
            image: require('./fightclub.png'),// Replace with actual image path
            url: 'https://mega.nz/file/5PUBzL4B#ndxqhMFKFrEWMdRVNOJV4Oc9wQU9GU3Pv8efRsniH4o', 
          },
          {
            title: 'GOOD WILL HUNTING',
            image: require('./goodwillhunting.webp'), // Replace with actual image path
            url: 'https://mega.nz/file/xW0mxRCT#AJ897uoapsnj73Pl5WBW_tuDwychyCQwnxRGeF6X9PA', 
          },
          {
            title: 'Oppenheimer',
            image: require('./Oppenheimer.jpg'), // Replace with actual image path
            url: 'https://mega.nz/file/pXMHnIaT#WZTzxk5Dvf0ShNuawd_4ljO1AR67c4JcJHu6NIVF8F0', 
          },
          {
            title: 'The Big Short',
            image: require('./BigShort.jpg'), // Replace with actual image path
            url: 'https://mega.nz/file/1KFV1IzC#daxmsSbGEQ_EcAFn-MeRLueQavLwavEt_CpxmKXvrFY', 
          },
          {
            title: 'Into the Wild',
            image: require('./IntoTheWild.jpg'), // Replace with actual image path
            url: 'https://mega.nz/file/NfUnxQ7J#d7LJcJAxJI0LE15gtCqVa4LVuoU6KTcgfZ6aOnXrn4U', 
          },
          {
            title: 'Whiplash',
            image: require('./Whiplash.jpg'), // Replace with actual image path
            url: 'https://mega.nz/file/5f1xyTIK#XdIcQJ4IaMs_70PHu4oDVEWm059Z_Qk04QS07VyufY4', 
          },
          {
            title: 'Wolf of Walll Street',
            image: require('./WolfOfWallStreet.jpg'), // Replace with actual image path
            url: 'https://mega.nz/file/padyyb6B#M85w9eYgU1LnQowXAS_Kkk6R0cwXX9JumXvhWdWoKA0', 
          },
          {
            title: 'Moneyball',
            image: require('./MoneyBall.jpg'), // Replace with actual image path
            url: 'https://mega.nz/file/lK1VmCwA#CAl1SmK4saryR59modqbMRtIf32JMlQnnUGV3jY_5Z0', 
          },
          {
            title: 'Superbad',
            image: require('./Superbad.jpg'), // Replace with actual image path
            url: 'https://mega.nz/file/9eFQFQbD#QOZb5EOqNmNZ3_2LtmD2fbYu9SFA59lgVSiQVok2pKY', 
          },
    ];
  
    static playMovie(url) {
      window.open(url, '_blank');
    }
  }
  
  export default Movies;